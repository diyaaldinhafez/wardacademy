...
function relTime(iso: string, nowMs: number) {
  ...
  if (d <= 0) return "جارية الآن";
  const mins = Math.round(d / 60000);
  if (mins < 60) return `بعد ${mins} دقيقة`;
  const hrs = Math.round(mins / 60);
  return `بعد ${hrs} ساعة${hrs > 1 ? "" : ""}`;
}
...
export default async function TodayPage() {
  ...
  const nm = (id: string | null) => (id ? nameOf.get(id) ?? "طالب" : "طالب");
  ...
  for (const s of (pastSessions ?? []) as any[]) {
    ...
    if (!rep) tasks.push({ key: `rep-${s.id}`, label: `اكتب تقرير جلسة — ${nm(s.learner_id)}`, learnerId: s.learner_id, tone: "warning", href: to });
    else if (rep.status === "draft") tasks.push({ key: `repd-${s.id}`, label: `اعتمِد تقرير جلسة — ${nm(s.learner_id)}`, learnerId: s.learner_id, tone: "warning", href: to });
  }
  ...
  for (const h of (toGrade ?? []) as any[]) tasks.push({ key: `hw-${h.id}`, label: `صحّح واجب «${h.title}» — ${nm(h.learner_id)}`, learnerId: h.learner_id, tone: "brand", href: `/studio/students/${h.learner_id}?tab=homework` });

  const { data: assessmentDrafts } = await supabase.from("assessments").select("id, learner_id, title").eq("status", "draft");
  for (const a of (assessmentDrafts ?? []) as any[]) tasks.push({ key: `as-${a.id}`, label: `اعتمِد اختبار «${a.title}» — ${nm(a.learner_id)}`, learnerId: a.learner_id, tone: "neutral", href: `/studio/students/${a.learner_id}?tab=assessments` });
  ...
  for (const it of (itemDrafts ?? []) as any[]) tasks.push({ key: `it-${it.id}`, label: `راجِع واجباً رقمياً — ${nm(it.target_learner_id)}`, learnerId: it.target_learner_id, tone: "neutral", href: it.target_learner_id ? `/studio/students/${it.target_learner_id}?tab=homework` : "/studio/students" });

  const attention = new Set(tasks.map((t) => t.learnerId).filter(Boolean) as string[]);
  const remainingToday = (todaySessions ?? []).filter((s: any) => sEnd(s) > nowMs).length;

  const stats = [
    { label: "جلسات اليوم", value: (todaySessions ?? []).length, tone: "var(--brand)" },
    { label: "المتبقّي اليوم", value: remainingToday, tone: "var(--leaf-700)" },
    { label: "مهامّ معلّقة", value: tasks.length, tone: tasks.length ? "var(--apricot-600, #c97a2b)" : "var(--text-muted)" },
    { label: "طلابك", value: learners.length, tone: "var(--text-strong)" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 320px), 1fr))", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {hero ? (
            <Card style={{ display: "flex", flexDirection: "column", gap: 12, background: "linear-gradient(135deg, var(--brand-50, #f1ecff), var(--surface-card))", border: "1.5px solid var(--brand-200, #d8ccff)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 12.5, fontWeight: 800, color: ongoing ? "var(--rose-600, #c0392b)" : "var(--brand)" }}>{ongoing ? "🔴 جلسةٌ جارية الآن" : "🎥 جلستك القادمة"}</span>
                <Badge tone="neutral">{relTime(hero.scheduled_at, nowMs)}</Badge>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--text-muted)", fontVariantNumeric: "tabular-nums" }}>
                    {timeOf(hero.scheduled_at)} · {hero.duration_minutes} دقيقة{hero.lesson_title ? ` · 📖 ${hero.lesson_title}` : ""}
                  </div>
                </div>
              </div>
              {heroJoinable ? (
                ...
                <Link href={`/studio/students/${hero.learner_id}?tab=sessions`} className="ward-btn ward-btn--secondary ward-btn--md" style={{ alignSelf: "flex-start" }}>حضّر الدرس ←</Link>
              )}
            </Card>
          ) : (
          ...
          <Card>
            ...
            {(todaySessions ?? []).length === 0 && <p style={{ fontSize: 13, color: "var(--text-muted)" }}>لا جلسات اليوم.</p>}
            {(todaySessions ?? []).map((s: any, i: number) => {
              ...
              return (
                <div key={s.id} style={{ ...row, ...(i === (todaySessions ?? []).length - 1 ? { borderBottom: "none" } : {}), opacity: state === "ended" ? 0.6 : 1 }}>
                  ...
                  {state === "ended" ? <Badge tone="neutral">انتهت</Badge> : state === "ongoing" ? <Badge tone="success">جارية</Badge> : null}
                  <Link href={`/studio/students/${s.learner_id}?tab=sessions`} className={btnSm}>التفاصيل</Link>
                </div>
              );
            })}
          </Card>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <Card variant="soft" style={{ borderColor: "var(--ward-purple-200)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: "var(--ward-purple-800)" }}>مهامّك اليوم</span>
              {tasks.length > 0 && <Badge tone="brand">{tasks.length}</Badge>}
            </div>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>كلّ ما ينتظر فعلك، في مكانٍ واحد — لا يصل شيءٌ للطالب قبل اعتمادك.</p>
            {tasks.length === 0 ? (
              <p style={{ fontSize: 13, color: "var(--leaf-700)" }}>لا مهامّ معلّقة. أحسنتِ ✓</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {tasks.slice(0, 12).map((t) => (
                  <Link key={t.key} href={t.href} style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--surface-card)", borderRadius: 12, padding: "10px 12px", border: "1px solid var(--ward-purple-100)", textDecoration: "none" }}>
                    <span style={{ width: 8, height: 8, borderRadius: 999, flexShrink: 0, background: t.tone === "warning" ? "var(--apricot-400)" : t.tone === "brand" ? "var(--brand)" : "var(--ink-300, #c9c4d6)" }} />
                    <span style={{ flex: 1, fontSize: 13, color: "var(--text-body)" }}>{t.label}</span>
