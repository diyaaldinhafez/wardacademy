...
export default function AvailabilityMatrix({ rules }: { rules: Rule[] }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {/* Time axis (LTR: early → late) */}
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ width: 56, flexShrink: 0 }} />
        <div dir="ltr" style={{ position: "relative", flex: 1, height: 14 }}>
          {ticks.map((t) => (
      ...
      {WEEKDAY_AR.map((label, wd) => {
        ...
        return (
          <div key={wd} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ width: 56, flexShrink: 0, fontSize: 12.5, fontWeight: 600, color: windows.length ? "var(--text-strong)" : "var(--text-muted)" }}>{label}</span>
            <div dir="ltr" style={{ position: "relative", flex: 1, height: 22, borderRadius: 8, background: "var(--surface-sunken)", overflow: "hidden" }}>
              {ticks.map((t) => (
              ...
              {windows.map((w, i) => (
                ...
                >
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>{fmt(w.start)}–{fmt(w.end)}</span>
                </span>
              ))}
            </div>
          </div>
